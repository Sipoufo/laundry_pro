...
const AlertMessageComponent = ({ title, message, isActive, setIsActive, isError = false, arrayError }) => {
    const result = [];
    
    const arrayReturn = () => {
        if (arrayError) {
            var key = 0;
            for (const input in arrayError) {
                result.push(
                    <li key={key}>{input} : {arrayError[input]}</li>
                )
                key++;
            }
        }
