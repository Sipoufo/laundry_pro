...
const CustomerModalUpdate = ({ active, setActive, productId }) => {
    ...
    const fetch = async (id) => {
        ...
        if (data.data) {
            ...
            setNormalIroning(data.data["lastName"]);
            setFastIroning(data.data["phone"]);
            setNormalDetergent(data.data["address"]);
            setFastDetergent(data.data["lastName"]);
            setNormalGloriousPressing(data.data["phone"]);
            setFastGloriousPressing(data.data["address"]);
        }
    };
    ...
    return (
        ...
        >
            <div
                className={`${
                    !active && "hidden"
                } absolute flex justify-center items-center w-screen h-screen top-0 left-0 bg-black bg-opacity-70`}
